...
GUEST_IMAGE=/home/tdx_hy/td-guest-centos-stream-8.hongyu.extend.qcow2
MEM=1G
...
$QEMU_IMAGE \
  ...
  -netdev user,id=mynet0,hostfwd=tcp::10028-:22,hostfwd=tcp::12028-:2375 \
  -device vhost-vsock-pci,guest-cid=28 \
  ...
  -serial chardev:mux \
	...
	-kernel ${KERNEL_IMAGE} \
	-append "root=/dev/vda3 ro console=hvc0 earlyprintk=ttyS0 ignore_loglevel debug earlyprintk l1tf=off initcall_debug log_buf_len=200M swiotlb=force tsc=reliable mce=off efi=noruntime efi=debug nokaslr" \
	-monitor pty \
  -monitor telnet:127.0.0.1:9028,server,nowait \
  -no-hpet \
  -nodefaults \
